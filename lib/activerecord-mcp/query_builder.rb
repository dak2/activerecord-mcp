...
module ActiveRecordMcp
  class QueryBuilder
    ...
    def fetch_records(natural_language_query)
      ...
      begin
        ...
        
        {
          ...
          query: parsed_query,
          count: results.respond_to?(:count) ? results.count : (results.is_a?(Array) ? results.size : 1),
    ...
    def load_rails_environment
      ...
      if File.exist?(File.join(rails_root, "config", "application.rb"))
    ...
    def extract_conditions(query)
      ...
      if query.match(/id\s*=\s*(\d+)/) || query.match(/with id\s+(\d+)/)
        conditions[:id] = $1.to_i
      end
    ...
    def find_model_class(model_name)
      ...
      begin
        ...
        %w[User Post Article Comment Product Order].each do |common_model|
          if common_model.downcase == model_name.downcase
            return common_model.constantize rescue nil
          end
        end
        nil
      end
    ...
    def execute_query(query, parsed_query)
      # Execute the query
      query.to_a
    end
