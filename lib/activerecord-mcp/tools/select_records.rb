...
module ActiveRecordMcp
  module Tools
    class SelectRecords < MCP::Tool
      ...
      def self.call(server_context:, model_name: nil, filter_condition: nil, order_by: nil, limit: nil,
                    count_only: false)
      ...
      def self.capture3_args_for(model_name: nil, filter_condition: nil, order_by: nil, limit: nil, count_only: false,
                                 dir: projects_root_file_path)
      ...
      def self.build_query_chain(model_class, filter_condition, order_by, limit, count_only)
        ...
        query += ".where(\"#{filter_condition}\")" if filter_condition

        query += ".order(\"#{order_by}\")" if order_by && !count_only

        query += ".limit(#{limit})" if limit && !count_only
